            ...
            <Layout>
                <Sider hide-trigger :style="{background: '#fff'}">
                    <Menu active-name="1-1" theme="light" width="auto" :open-names="['1']">
                        <Submenu :name="menu.systemName" v-for="(menu, index) in subSystem" :key="index">
                            <template slot="title">
                                <Icon type="menu.systemIcon"></Icon>
                                {{menu.systemTitle}}
                            </template>
                            <MenuItem :name="page.pageName" v-for="(page, index) in menu.systemPages"
                             :key="index">{{page.pageTitle}}</MenuItem>
                        </Submenu>
                    </Menu>
                </Sider>
